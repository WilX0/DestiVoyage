...
include_once("testvols1.php");//la session est dans testvols1.php
if (isset($_GET['alert'])) {
...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["search"])) :
    ...
    ?>
    <div class="container">
                <!-- <h2 class="text-center mt-4">Aller</h2> -->

                <div id="result-container" class="flight-list">
                    <?php
                    if (isset($_GET['dateretour']) && !empty($_GET['dateretour'])) {
                        ?>
                        <!-- <h2 class="text-center mt-4">Retour</h2> -->
                        <?php
...
//  document.getElementById("dateret").valueAsDate = new Date(getCurrentDate()); 
...
function verifform(event){
    ...
    if(dqt1>dqt2 && allerRetourRadio2.checked){
        event.preventDefault();
...
//     var arrivee = document.getElementById("arr").value;
//     var dateDepart = new Date(document.getElementById("datedepart").value);
//     var dateRetour = new Date(document.getElementById("dateretour").value);
//     var allerRetourRadio = document.getElementById("allerretour");

//     // Vérification si le lieu de départ et d'arrivée est le même
//     if (userLocation === arrivee) {
//         alert("Le lieu de départ et d'arrivée ne peut pas être le même.");
//         event.preventDefault(); // Empêche l'envoi du formulaire
//         return;
//     }

//     // Vérification si la date de départ est ultérieure à la date de retour (s'il s'agit d'un aller-retour)
//     if (allerRetourRadio.checked && dateDepart >= dateRetour) {
