...
/**
 * @file
 * Page de profil de l'utilisateur, affichant les informations utilisateur et les vols favoris.
 * 
 * PHP version 7.3.11
 * 
 * @category Profil
...
if (isset($_SESSION["login"])) {
    // Afficher un message de succès si le mot de passe est mis à jour
    if (isset($_GET['success']) && $_GET['success'] == 1) {
        echo '<div class="alert alert-success">Mot de passe mis à jour avec succès.</div>';
    }
    // Afficher un message de succès si l'image est mis à jour
    if (isset($_GET['uploadStatusImage'])) {
        $uploadStatusImage = htmlspecialchars($_GET['uploadStatusImage']);
        echo "<p>$uploadStatusImage</p>";
             echo "<script>
        history.replaceState({}, document.title, window.location.pathname);
             </script>";
    }
    // Afficher un message de succès si la description est mis à jour
    if (isset($_GET['uploadStatusDescription'])) {
        $uploadStatusDescription = htmlspecialchars($_GET['uploadStatusDescription']);
        echo "<p>$uploadStatusDescription</p>";
             echo "<script>
        history.replaceState({}, document.title, window.location.pathname);
             </script>";
    }
    ...
    $queryUserInfo = "SELECT * FROM user WHERE login = :userId";
...
$stmtRegisteredFlights->bindParam(':userId', $mailuser, PDO::PARAM_STR);
...
<body id="helo">
 <?php
if (isset($_SESSION["login"])) {
     // Inclusion de la barre de navigation si l'utilisateur est connecté
        ...
        <section class="main-section">
            <div class="user-description">
                ...
                <form action="update_profil.php" method="post" enctype="multipart/form-data">
                    <!-- Champ pour choisir une image de profil -->
                <label for="profileImage">Image de profil :</label>

                
                <?php
                // Vérification si l'utilisateur a déjà une image de profil
                if(isset($user['photo'])){
                $userImage = $user['photo'];
                }
                // Affichage de l'image de profil actuelle s'il y en a une
if (!empty($userImage)) {
...
<!-- Champ pour télécharger une nouvelle image de profil -->
    ...
    <footer><?php include("../include/lefooter.php"); ?></footer>
